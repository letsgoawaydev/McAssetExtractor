...
public final class McAssetExtractor {
    ...
    public static String status = "Downloading Client Jar....";

    public void run(String version, File outDir) {
	...
	setStatus("Extracting client assets...");
	int[] jarResult = extractJarAssets(clientJar, outDir);
	setStatus("Getting asset index...");
	...
	status = message;
	UserInterface.ui.pathLabel.setText(message);
	UserInterface.ui.reload();
    }
	...
	for (JsonElement elem : versions)
	{
	    ...
	    versionsList.add(obj.get("id").getAsString());
	}
	return versionsList;
    }
	...
	return manifest.getAsJsonObject("latest").get("snapshot").getAsString();
    }
	...
	setStatus("Finished! Extracted " + (jar[0] + meta[0]) + " assets!");
    ...
    private static JsonElement downloadJson(String url) throws IOException {
